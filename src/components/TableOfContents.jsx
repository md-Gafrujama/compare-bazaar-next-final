...
const TableOfContents = ({ contents }) => {
  const [tocItems, setTocItems] = useState(
    contents.map((item) => ({ ...item, active: false }))
  );
  ...
  return (
    <div className="lg:w-80 lg:sticky lg:top-24 lg:self-start">
      <div className="bg-white rounded-4xl shadow-sm border border-gray-200 p-6 flex flex-col" style={{ scrollBehavior: "smooth" }}>
        <h2 className="text-lg font-semibold text-gray-900 mb-4">
          Table of contents
        </h2>

        {/* Scrollable TOC only if more than 5 */}
        <nav
          className="space-y-2 overflow-y-auto pr-1"
          style={{
            maxHeight: contents.length > 5 ? "240px" : "auto",
          }}
        >
